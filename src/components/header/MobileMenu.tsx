...
interface MobileMenuProps {
  ...
  onNavLinkClick: (section: string, path: string) => void;
}
    ...
    <div className="md:hidden bg-background/95 backdrop-blur-lg shadow-lg animate-in slide-in-from-top duration-300">
      <div className="container py-4 flex flex-col space-y-2">
        {navItems.map((item) => (
          <button
            key={item.id}
            onClick={() => onNavLinkClick(item.section, item.path)}
            className={`px-4 py-3 rounded-md transition-all duration-300 relative text-left ${
